...
| `10 Areas/Rule Opener Note.md` | an unterminated comment opener on a line that is otherwise a horizontal rule, with nothing else open. Both branches that could destroy the line are bypassed: last_closing_line returns 0, so the opener is literal from the outset, the line is never truncated, and the rule pattern is never applied to it |
